...
static double uni(void) {
  ...
  return (double)((rs >> 11) & 0x1FFFFFFFFFFFFFULL) /
...
static void check_tree_integrity(const sif_octree_t* tree,
  const sif_field_t* field, uint32_t node, sif_real cx, sif_real cy,
  sif_real cz, sif_real hs, uint8_t* seen, int* bad_containment) {
  ...
  const sif_real q = hs * 0.5f;
  for (int i = 0; i < 8; i++) {
    check_tree_integrity(tree, field, nd->first_child + i,
      cx + ((i & 1) ? q : -q), cy + ((i & 2) ? q : -q), cz + ((i & 4) ? q : -q),
      q, seen, bad_containment);
  }
}

static void run(const char* label, sif_real sx, sif_real sy, sif_real sz,
  ...
  sif_octree_t* tree = sif_octree_alloc(f, max_per_leaf);
  CHECK(tree != NULL, "octree build failed");
  if (!tree)
    return;
  ...
  CHECK(
    missing == 0, "%llu particles are in no leaf", (unsigned long long)missing);
  ...
  for (int q = 0; q < n_queries; q++) {
    const sif_real px = (sif_real)(uni() * sx);
    const sif_real py = (sif_real)(uni() * sy);
    const sif_real pz = (sif_real)(uni() * sz);
    ...
    if (gd2 > best * (1.0 + 1e-6)) {
      ...
      if (wrong <= 2)
        printf(
          "    query %d: got d=%.6f, brute force d=%.6f (idx %llu vs %llu)\n",
...
int main(void) {
  ...
  run(
    "extreme aspect", 500.0f, 3.0f, 60.0f, (uint64_t)SIF_TEST_SCALE(20000), 8);
