...
int main(void) {
  ...
  sif_profiles_mesh(
    cat, f, BOX, ext, N_BINS, SIF_PBC_PERIODIC, &dens, NULL);
  ...
  const real_t r_max = vr * ext;
  const real_t half = BOX * 0.5f;
  ...
  for (uint32_t b = 0; b < N_BINS; b++) {
    ...
    for (uint64_t i = 0; i < N_P; i++) {
      ...
      if (dx > half) dx = BOX - dx;
      if (dy > half) dy = BOX - dy;
      if (dz > half) dz = BOX - dz;
      if (dx * dx + dy * dy + dz * dz <= r_outer * r_outer)
        n_in++;
    }

    const double vol = (4.0 / 3.0) * M_PI * r_outer * r_outer * r_outer;
