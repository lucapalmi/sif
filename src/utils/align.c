...
void* sif_malloc_aligned(size_t size) {
  ...
  /* Pad size to a multiple of SIF_CACHE_LINE to prevent false sharing
   * between OpenMP threads in adjacent memory blocks. */
  size_t padded_size = (size + SIF_CACHE_LINE - 1) & ~(SIF_CACHE_LINE - 1);
  void* ptr = NULL;

#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 201112L &&                \
  !defined(__APPLE__)
  /* C11 Standard Aligned Allocator */
  ptr = aligned_alloc(SIF_CACHE_LINE, padded_size);
#else
  /* POSIX fallback (macOS and older Linux glibc) */
  if (posix_memalign(&ptr, SIF_CACHE_LINE, padded_size) != 0) {
    return NULL;
  }
#endif
...
void* sif_calloc_aligned(size_t count, size_t size) {
  ...
  if (ptr) {
    ...
    size_t padded_size =
      (total_size + SIF_CACHE_LINE - 1) & ~(SIF_CACHE_LINE - 1);
