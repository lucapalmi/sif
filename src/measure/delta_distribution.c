 ...
 */
static int histogram_field(const sif_real* data, uint64_t total_cells,
  const sif_real delta_bounds[2], uint32_t n_bins, sif_real inv_bin_width,
  sif_real* out, sif_real pdf_norm) {
  ...
  if (!scratch) {
    SIF_LOG_ERROR(TAG, "failed to allocate the histogram scratch");
    return SIF_ERR_ALLOC;
  }

  const sif_real lo = delta_bounds[0];
  const sif_real hi = delta_bounds[1];

#pragma omp parallel num_threads(n_threads)
  {
    uint64_t* local = scratch + (size_t)sif__system_thread_num() * n_bins;

#pragma omp for schedule(static)
    for (uint64_t i = 0; i < total_cells; i++) {
      const sif_real d = data[i];
  ...
  for (uint32_t b = 0; b < n_bins; b++) {
    ...
    out[b] = (sif_real)((double)total * (double)pdf_norm);
  }
...
sif_delta_distribution_t* sif_delta_distribution_grid(const sif_grid_t* grid,
  const sif_real* radii, uint32_t n_radii, uint32_t n_bins,
  const sif_real delta_bounds[2], uint64_t seed, sif_option opt) {

  if (!grid || !grid->values || !radii || !delta_bounds || n_radii == 0 ||
      n_bins == 0) {
    SIF_LOG_ERROR(TAG, "invalid arguments to sif_delta_distribution_grid");
    return NULL;
  }

  if (!(delta_bounds[0] < delta_bounds[1])) {
    SIF_LOG_ERROR(
      TAG, "delta_bounds[0] must be strictly less than delta_bounds[1]");
    return NULL;
  }
  ...
  sif_delta_distribution_t* dist =
    sif__delta_distribution_alloc(n_radii, n_bins);
  if (!dist)
    return NULL;

  memcpy(dist->radii, radii, (size_t)n_radii * sizeof(sif_real));
  dist->n_samples = grid->total_cells;

  const sif_real bin_width =
    (sif_real)(((double)delta_bounds[1] - (double)delta_bounds[0]) / n_bins);
  const sif_real inv_bin_width = (sif_real)(1.0 / (double)bin_width);

  for (uint32_t i = 0; i < n_bins; i++) {
    dist->delta_edges[i] = delta_bounds[0] + (sif_real)i * bin_width;
  }
  ...
  const sif_real pdf_norm =
    (sif_real)(1.0 / ((double)grid->total_cells * (double)bin_width));

  for (uint32_t k = 0; k < n_radii; k++) {
    if (sif__fft_apply_filter(ws, filter, radii[k], grid->box_length) !=
        SIF_OK) {
      SIF_LOG_ERROR(TAG, "failed to apply the filter at radius %u", k);
      goto fail;
    }

    /* Runs in place on delta_k_cpy, which the next iteration's filter fully
     * overwrites from the untouched delta_k. */
    sif_real* filtered = sif__fft_grid_backward(ws);
    if (!filtered) {
      SIF_LOG_ERROR(TAG, "backward transform failed at radius %u", k);
      goto fail;
    }

    if (histogram_field(filtered, grid->total_cells, delta_bounds, n_bins,
  ...
  SIF_LOG_INFO(TAG, "delta distribution computation complete");
