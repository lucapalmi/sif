...
static inline double window(bool gaussian, double x) {
  if (gaussian)
    return exp(-0.5 * x * x);
...
static int validate_pk_table(
  const sif_real* k, const sif_real* pk, uint32_t n_points) {

  if (!k || !pk || n_points < 2) {
    SIF_LOG_ERROR(TAG, "the P(k) table needs at least two samples");
    return SIF_ERR_INVALID;
  }
  ...
  for (uint32_t i = 0; i < n_points; i++) {
    ...
    if (i > 0 && !(k[i] > k[i - 1])) {
      SIF_LOG_ERROR(TAG, "k is not strictly increasing at index %u", i);
      return SIF_ERR_INVALID;
    }
  }
...
sif_delta_moments_t* sif_delta_moments_pk(const sif_real* k, const sif_real* pk,
  uint32_t n_points, const sif_real* radii, uint32_t n_radii, uint8_t order,
  sif_option opt) {

  if (!radii || n_radii == 0) {
    SIF_LOG_ERROR(TAG, "invalid arguments to sif_delta_moments_pk");
    return NULL;
  }
  ...
  if (validate_pk_table(k, pk, n_points) != SIF_OK)
    return NULL;
  ...
  const bool gaussian =
    (opt & SIF__DELTA_FILTER_MASK) == SIF_DELTA_FILTER_GAUSSIAN;
  ...
  const double prefactor = 1.0 / (2.0 * SIF_PI * SIF_PI);
...
#pragma omp parallel for schedule(static)
  for (uint32_t r = 0; r < n_radii; r++) {
    ...
    for (uint32_t i = 0; i < n_points; i++) {
      ...
      const double w = window(gaussian, ki * R);
    ...
    for (uint8_t j = 0; j < n_moments; j++) {
      ...
      m->sigma[at] = (sif_real)(s > 0.0 ? sqrt(s) : 0.0);
      m->high_k_fraction[at] =
        (sif_real)(total[j] > 0.0 ? high[j] / total[j] : 0.0);
    }
  }
  ...
  for (uint32_t r = 0; r < n_radii; r++) {
    for (uint8_t j = 0; j < n_moments; j++) {
      ...
      if (m->high_k_fraction[at] > HIGH_K_WARN_LEVEL) {
        SIF_LOG_WARNING(TAG,
  ...
  SIF_LOG_INFO(TAG, "sigma_0..sigma_%u evaluated from P(k)", order);
 ...
 * around 1e-10.
 */
static inline double window_slope(bool gaussian, double x) {
  if (gaussian)
...
sif_real* sif_delta_sigma_slope_pk(const sif_real* k, const sif_real* pk,
  uint32_t n_points, const sif_real* radii, uint32_t n_radii, sif_option opt) {

  if (!radii || n_radii == 0) {
    SIF_LOG_ERROR(TAG, "invalid arguments to sif_delta_sigma_slope_pk");
    return NULL;
  }
  ...
  sif_real* out = sif_calloc_aligned((size_t)n_radii, sizeof(sif_real));
  if (!out) {
    SIF_LOG_ERROR(TAG, "failed to allocate the slope array");
    return NULL;
  }
...
#pragma omp parallel for schedule(static)
  for (uint32_t r = 0; r < n_radii; r++) {
    ...
    out[r] = (sif_real)(denominator > 0.0 ? numerator / denominator : 0.0);
  }
 ...
 * semi-definite. */
static double log_trapezoid_weight(const double* logk, uint32_t n, uint32_t m) {
...
double* sif_delta_covariance_pk(const sif_real* k, const sif_real* pk,
  uint32_t n_points, const sif_real* radii, uint32_t n_radii, sif_real* sigma,
  sif_real* high_k_fraction, double* deriv_variance, sif_option opt) {
  ...
  double* block =
    sif_malloc_aligned((size_t)n_radii * COV_K_BLOCK * sizeof(double));
  ...
  for (uint32_t m = 0; m < n_points; m++) {
    ...
    const double w = log_trapezoid_weight(logk, n_points, m);
    amp[m] = sqrt(w * ki * ki * ki * (double)pk[m] * prefactor);
  }

  const double k_high = 0.5 * (double)k[n_points - 1];

  for (uint32_t base = 0; base < n_points; base += COV_K_BLOCK) {
    const uint32_t len =
      (n_points - base < COV_K_BLOCK) ? n_points - base : COV_K_BLOCK;

    /* The only transcendentals in the whole routine. */
#pragma omp parallel for schedule(static)
    for (uint32_t i = 0; i < n_radii; i++) {
      const double R = (double)radii[i];
      double* Ai = block + (size_t)i * COV_K_BLOCK;

      for (uint32_t m = 0; m < len; m++)
        Ai[m] = amp[base + m] * window(gaussian, (double)k[base + m] * R);
      ...
      for (uint32_t m = 0; m < len; m++) {
        ...
        Bi[m] = amp[base + m] * ki * window_slope(gaussian, ki * R);
...
#pragma omp parallel for schedule(dynamic, 4)
    for (uint32_t i = 0; i < n_radii; i++) {
      const double* Ai = block + (size_t)i * COV_K_BLOCK;

      for (uint32_t j = 0; j <= i; j++) {
        const double* Aj = block + (size_t)j * COV_K_BLOCK;
  ...
  for (uint32_t i = 0; i < n_radii; i++) {
    ...
    if (sigma)
      sigma[i] = (sif_real)(diag > 0.0 ? sqrt(diag) : 0.0);
    if (high_k_fraction)
      high_k_fraction[i] = (sif_real)(diag > 0.0 ? high[i] / diag : 0.0);
    ...
    if (!(diag > 0.0)) {
      SIF_LOG_WARNING(TAG,
        ...
        (double)radii[i]);
    } else if (high[i] / diag > HIGH_K_WARN_LEVEL) {
      SIF_LOG_WARNING(TAG,
