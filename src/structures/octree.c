...
sif_octree_t* sif_octree_alloc(uint32_t initial_capacity) {
  ...
  tree->nodes = malloc(initial_capacity * sizeof(sif_octree_node_t));
  if (!tree->nodes) {
    SIF_LOG_ERROR("octree", "failed to allocate octree nodes (%zu bytes)",
      initial_capacity * sizeof(sif_octree_node_t));
    free(tree);
    return NULL;
  }
  ...
  tree->root_center[0] = 0;
  tree->root_center[1] = 0;
  tree->root_center[2] = 0;
  tree->root_half_span = 0;
...
void sif_octree_free(sif_octree_t* tree) {
  ...
  SIF_LOG_WARNING("octree", "cannot free an unitialized octree");
}
 ...
 */
static void __octree_subdivide(sif_octree_t* tree, uint32_t node_idx,
  const sif_field_t* field, real_t cx, real_t cy, real_t cz, real_t half_span,
  uint32_t max_per_leaf, uint32_t depth) {
  ...
  if (tree->count + 8 > tree->capacity) {
    ...
    if (!new_nodes) {
      SIF_LOG_ERROR("octree", "failed to grow the node array to %u entries",
        new_capacity);
      return;
    }
  ...
  for (int i = 0; i < 8; i++) {
    tree->nodes[first_child_idx + i].first_child = __OCTREE_LEAF_NODE;
  ...
  const real_t origin_x = tree->root_center[0] - tree->root_half_span;
  const real_t origin_y = tree->root_center[1] - tree->root_half_span;
  const real_t origin_z = tree->root_center[2] - tree->root_half_span;
  const real_t inv_side = 1.0f / (2.0f * tree->root_half_span);
  ...
  for (uint32_t i = start_idx; i < end_idx; i++) {
    ...
    const uint8_t octant = (uint8_t)((((qx >> bit) & 1u) << 0) |
                                     (((qy >> bit) & 1u) << 1) |
                                     (((qz >> bit) & 1u) << 2));
    counts[octant]++;
  }
  ...
  real_t quarter_span = half_span * 0.5f;
  for (int i = 0; i < 8; i++) {
    if (tree->nodes[first_child_idx + i].p_counting > max_per_leaf) {
      real_t child_cx = cx + ((i & 1) ? quarter_span : -quarter_span);
      real_t child_cy = cy + ((i & 2) ? quarter_span : -quarter_span);
      real_t child_cz = cz + ((i & 4) ? quarter_span : -quarter_span);

      __octree_subdivide(tree, first_child_idx + i, field, child_cx, child_cy,
        child_cz, quarter_span, max_per_leaf, depth + 1);
    }
  }
}

int sif_octree_build(
  sif_octree_t* tree, sif_field_t* field, uint32_t max_per_leaf) {
  if (!tree || !field || field->n_particles == 0) {
    SIF_LOG_ERROR("octree", "invalid input octree or field");
    return -1;
  }
  ...
  tree->count = 1;
  tree->nodes[0].first_child = __OCTREE_LEAF_NODE;
  tree->nodes[0].p_start = 0;
  tree->nodes[0].p_counting = field->n_particles;
  tree->nodes[0].padding = 0;

  __octree_subdivide(tree, 0, field, tree->root_center[0], tree->root_center[1],
...
static inline real_t __sq_dist_point_aabb(
  real_t px, real_t py, real_t pz, real_t cx, real_t cy, real_t cz, real_t hs) {
  real_t min_x = cx - hs, max_x = cx + hs;
  real_t min_y = cy - hs, max_y = cy + hs;
  real_t min_z = cz - hs, max_z = cz + hs;
...
uint64_t sif_octree_find_nearest(const sif_octree_t* tree,
  const sif_field_t* field, real_t px, real_t py, real_t pz) {
  ...
  _octree_stack_t stack[8 * __OCTREE_MAX_DEPTH];
  int32_t sp = 0;
  ...
  real_t best_dist2 = REAL_MAX_VAL;
  uint64_t best_p = UINT64_MAX;

  while (sp > 0) {
    const _octree_stack_t cur = stack[--sp];

    /* The bound may have tightened since this entry was pushed. */
    if (__sq_dist_point_aabb(px, py, pz, cur.cx, cur.cy, cur.cz,
          cur.half_span) >= best_dist2) {
      continue;
    }

    const sif_octree_node_t* nd = &tree->nodes[cur.node_idx];

    if (nd->first_child == __OCTREE_LEAF_NODE) {
      const uint32_t end = nd->p_start + nd->p_counting;
      for (uint32_t i = nd->p_start; i < end; i++) {
        const real_t dx = field->x[i] - px;
        const real_t dy = field->y[i] - py;
        const real_t dz = field->z[i] - pz;
    ...
    const real_t q = cur.half_span * 0.5f;
    ...
    for (int pass = 0; pass < 2; pass++) {
      for (int i = 0; i < 8; i++) {
        ...
        const real_t ccx = cur.cx + ((i & 1) ? q : -q);
        const real_t ccy = cur.cy + ((i & 2) ? q : -q);
        const real_t ccz = cur.cz + ((i & 4) ? q : -q);

        if (__sq_dist_point_aabb(px, py, pz, ccx, ccy, ccz, q) >= best_dist2)
          continue;
...
uint64_t sif_octree_search_radius(const sif_octree_t* tree,
  const sif_field_t* field, real_t px, real_t py, real_t pz, real_t radius,
  uint64_t* out_indices, uint64_t max_capacity) {
  ...
  _octree_stack_t stack[__OCTREE_MAX_DEPTH];
  int top = 0;

  stack[top++] = (_octree_stack_t){0, tree->root_center[0],
    tree->root_center[1], tree->root_center[2], tree->root_half_span};

  while (top > 0) {
    _octree_stack_t curr = stack[--top];
    ...
    if (node->first_child == UINT32_MAX) {
      ...
      for (uint32_t i = start; i < end; i++) {
        real_t dx = field->x[i] - px;
        real_t dy = field->y[i] - py;
        real_t dz = field->z[i] - pz;
      ...
      real_t q_span = curr.half_span * 0.5f;
      for (int i = 0; i < 8; i++) {
        ...
        real_t child_cx = curr.cx + ((i & 1) ? q_span : -q_span);
        real_t child_cy = curr.cy + ((i & 2) ? q_span : -q_span);
        real_t child_cz = curr.cz + ((i & 4) ? q_span : -q_span);

        if (__sq_dist_point_aabb(
              px, py, pz, child_cx, child_cy, child_cz, q_span) <= radius2) {
          stack[top++] = (_octree_stack_t){
...
uint64_t sif_octree_search_box(const sif_octree_t* tree,
  const sif_field_t* field, real_t min_x, real_t min_y, real_t min_z,
  real_t max_x, real_t max_y, real_t max_z, uint64_t* out_indices,
  ...
  while (top > 0) {
    ...
    if (node->first_child == UINT32_MAX) {
      ...
      for (uint32_t i = start; i < end; i++) {
        real_t px = field->x[i], py = field->y[i], pz = field->z[i];
